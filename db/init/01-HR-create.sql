 ...
 *******************************************************/
IF DB_ID(N'IPL_GIS_DB26') IS NULL
BEGIN
  CREATE DATABASE IPL_GIS_DB26;
END
GO

-- Delay the script conclusion waiting for server and database to be online
-- This delay is critical for container boot
WHILE DB_ID(N'IPL_GIS_DB26') IS NOT NULL
  AND CAST(DATABASEPROPERTYEX(N'IPL_GIS_DB26','Status') AS NVARCHAR(60)) <> N'ONLINE'
BEGIN
